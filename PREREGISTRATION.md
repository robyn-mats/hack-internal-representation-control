...
| Date | Change | Reason |
|---|---|---|
| 2026-09-05 | **`informative_n` counting bug fixed**, before running `--compliant-only` on held-out. `informative()` previously used a looser missingness rule (`dropna(how="all")` + `fillna(0)`) than the tests it describes (`paired()`'s `(w[hi]-w[lo]).dropna()`, `omnibus()`'s `w[have].dropna()`), so a concept missing one side of a comparison but present and nonzero on the other could be counted "informative" even though it was excluded from the actual test. Fixed to require a complete case across the compared cells, matching the tests exactly — `informative_n` can no longer exceed the n a test actually used. Verified on a synthetic case (old code: 2, wrongly including a concept dropped by the real test; new code: 1, correct) and by re-running all four pilot smoke tests, which printed identically before and after (pilot's SAE missingness is concept-level, never per-cell, so the bug had no live case there — it was specifically a risk for `--compliant-only`, which filters per (concept, cell) independently and can strand one side of a pair). | Found on independent review, alongside the family-combination and `MIN_INFORMATIVE` decisions above. Left undocumented as a "check before trusting compliant-only" note until now; closing it before that run rather than after, since `--compliant-only` is one of the three registered invocations about to be run on held-out and is exactly the mode where the bug was live rather than theoretical. |
| 2026-09-05 | **New non-registered analysis variant: `--hybrid-compliant`.** For the held-out run, teacher-forcing was collected only for the trials that deviated in the generated pass (33 of them), specifically so this variant could be built: for every trial, use the generated-pass activation if it was compliant, and substitute the teacher-forced activation if it was not, rather than dropping the trial as `--compliant-only` does. Because a compliant trial's forced and generated activations are bit-identical (verified, max\|diff\| = 0), this is exactly "the main analysis, with only the non-compliant trials' activations replaced by their forced-to-comply equivalents" — every one of the 40 held-out concepts keeps its full trial count, and every trial in the resulting dataset reflects the model's activations while producing the registered carrier sentence, one way or the other. Implemented as a third mode in `14_confirmatory.py` alongside the primary (all trials) and `--compliant-only`; mutually exclusive with `--compliant-only`, and requires the (default) generated pass. Merges the already-computed `generated` and `teacher_forced` readout parquets by `prompt_group` — no new measurement, no GPU. Reported as a second robustness check alongside `--compliant-only`, not as a replacement for the primary (all-trials) analysis, and not added to the Holm family (it changes which activations feed the same 15 tests, not the tests themselves). | This was the original intent behind restricting held-out teacher-forcing to deviating trials (2026-08-31 amendment above) but was never implemented or written down as its own analysis mode — the earlier amendment only describes the surprisal-based method-validity check, which reports no confirmatory contrast at all. Distinct from `--compliant-only`: that variant *drops* a trial (and can shrink a concept out of a cell entirely if every trial for that cell happened to deviate); this variant *substitutes* a forced-compliant stand-in, so n is never reduced. Verified before writing: `artifacts/runs/heldout1/teacher_forced/results/` contains readouts for all 33 deviant prompt_groups (SAE and concept-vector, all four layers, both poolings), each independently confirmed to have been `exact_match = False` in the generated pass and `True` in the teacher-forced pass, i.e. a genuine substitute rather than a coincidental duplicate. Recorded before the held-out set is unblinded. |
| 2026-09-05 | **Two `14_confirmatory.py` implementation decisions refined, on independent review, before unblinding.** (a) **Interaction diagnostic added** to the four "mean"-combined family members (Q3c, Q5b, Q5c, Q5f): each now also reports the explicit 2x2 interaction contrast (`paired_interaction`) that Q5 and Q5d already carried, printed alongside the existing sub-comparisons. Not added to `FAMILY` — the Holm-corrected family stays at 15 members; this is diagnostic only, and changes no p-value that feeds the Holm correction. (b) **`MIN_INFORMATIVE` recomputed by exact noncentral-t power** instead of a `1/sqrt(n)` rescaling of the registered n=40 figure: **15 → 19** informative concepts required before a non-significant result may be called a null, at 80% power and the same Holm worst-case alpha (0.05/15) the registered power statement already uses. Every printed `detectable_dz` moves up slightly with it — e.g. the n=40 anchor itself, previously quoted as 0.60 by approximation, is 0.632 exactly. Both changes only make the analysis more conservative: neither can turn a result that was UNDERPOWERED into significant, or a null into non-null. | (a) Averaging the per-concept differences (the "mean" rule) is exactly the operation that cancels a sign-flipped interaction: a family member could read null by cancellation while both of its individual sub-comparisons are significant in opposite directions, with nothing to catch it. (b) The `1/sqrt(n)` rule was a normal-distribution approximation to a small-sample (noncentral-t) power problem; at the small n the floor effect produces, the approximation understates the dz required for 80% power, so it was mildly optimistic about calling a result a genuine null — the wrong direction for a rule whose entire purpose is to prevent overclaiming nulls. Both were found on independent review of the two decisions the script's own docstring flagged as unregistered (neither had been signed off); recorded with the worked arithmetic in `NOTES.md` 2026-09-05, and here before the held-out set is unblinded. |
| 2026-09-05 | **Floor-effect reporting, fixed before unblinding.** Three additions, none of which changes the registered tests, the primary readout, the analysis layer or the Holm family. (a) **Informative n** is reported for every contrast: the number of concepts with a non-zero readout in at least one of the cells compared. A concept reading exactly 0.0 in every cell of a contrast has a structurally zero difference and carries no information while still consuming an n. (b) **A non-significant result is reported as a null only when informative n >= 15**; below that it is labeled **UNDERPOWERED — absence of evidence, not evidence of absence**. The threshold is derived, not chosen: rescaling this file's own power figure (dz 0.60 at n=40, Holm across 15) as 1/sqrt(n), n=15 is the point below which the contrast could not detect even a large effect (dz 1.0). (c) The **concept-vector readout is reported for all 15 contrasts alongside the SAE**, with equal prominence rather than as a footnote. It remains the registered *secondary*; primacy is unchanged. Rank-based tests (Wilcoxon signed-rank, sign test) are **explicitly rejected** and may not be substituted later. | The SAE readout is 92.1% exactly zero at the committed layer 40 on the pilot, and the zeros are not spread evenly: measured per contrast on pilot data, informative concepts are 9 of 9 for Q0 and Q3c, but 5 of 9 for Q3 (the primary), 3 of 9 for Q1 and Q4, and **2 of 9 for Q5e** — the `irrelevant` vs `not relevant` comparison that motivates the fork. Cells directing attention toward the concept activate its latents; away and declarative cells largely do not. Without (a) and (b), most contrasts return non-significant and the available reading is "instruction framing does not change internal representation", which the data cannot support — the floor predicts the same p-values. The positive control (2026-08-31) already established these zeros are real measurements rather than a dead instrument, so this is about what the *analysis* may claim, not about instrument validity. Rank tests are rejected because Wilcoxon **discards zero differences**: on Q5e it would test 2 concepts while appearing more robust than the paired t, hiding the floor instead of handling it. Recorded before the held-out set was unblinded; all numbers cited are pilot-only. Implemented in `rk_scripts/14_confirmatory.py` as `informative()` and `verdict()`. |
| 2026-08-31 | **Q0 gate evaluation rule made explicit**, before held-out was measured or tested: the two adjacent steps must be **ordered by sign** (A > T1 and T1 > T7), and the **A-vs-T7 span** must be significant at alpha = .05 two-sided, uncorrected (the gate sits outside the Holm family). | This file states Q0 as "ordering holds" while saying "significantly" wherever it means significance (Q9: "L1 **significantly** below T6"; Q2: "no **significant** difference"), so the wording points to a directional ordering requirement rather than a significance test on each adjacent step. T1 is a graded middle term that was never powered as a separate test — the power calculation covers the primary contrast — so requiring significance on each step would gate the entire analysis on a comparison the design does not claim to resolve. Fixed now because the pilot cannot settle it: at n=9, A-vs-T7 has dz 0.711 and p = .066, and detecting T1-vs-T7's dz 0.324 would need n ≈ 78. The same dz 0.711 at the held-out n=37 gives p ≈ .0001, so the reading may not bind empirically — which is precisely why it is fixed while that is still unknown. |
| 2026-08-31 | **Within-member combination rule** for family members listing several comparisons, needed because the family is fixed at 15 without saying how a member with two or more comparisons yields one p-value. Adopted: **average of the per-concept differences** where a member's comparisons are directionally parallel (Q3c, Q5b, Q5c, Q5f), and a **Friedman repeated-measures omnibus** across the member's cells where they form a gradient or heterogeneous set that must not be averaged (Q5, Q5d, Q7, Q8). Every sub-comparison is additionally reported individually, and Q5/Q5d also report the explicit 2x2 interaction contrast ((I-G)-(K-M) and (A-C)-(B-E)). | Averaging is right for parallel contrasts — the same factor effect at two levels of another factor — and wrong for Q8, whose four comparisons ask different questions, or Q7, which is a gradient. Friedman avoids a sphericity assumption that 37 concepts across up to 7 cells cannot support. Recorded before held-out was tested. **This is an implementation decision the pre-registration does not cover, not a registered commitment**, and is flagged as such in `rk_scripts/14_confirmatory.py`. |
